...
namespace MDM.Client.IntegrationTests.Product
{
    ...
    {
        public static Product PostBasicEntity()
        {
            var mdmService = ServiceLocator.Current.GetInstance<IMdmEntityService<Product>>();
            ...
            Assert.IsTrue(response.IsValid, response.Code + " : " + (response.Fault == null ? string.Empty : response.Fault.Message + " : " + response.Fault.Reason));
        ...
        protected static void SetAdditionalData(Product entity)
        {
			
            entity.Details.Market = IntegrationTests.Market.MarketData.PostBasicEntity().ToNexusId().ToEntityId();
