...
namespace MDM.Client.IntegrationTests.SettlementContact.MdmService
{
    ...
    public class MdmServiceBasicIntegrationTests : MdmServiceIntegrationTestBase
    {
        ...
        public void ShouldGetByIntId()
        {
            ...
            Check(settlementcontact, response.Message);
        }
        ...
        {
            // given
            var nexusId = settlementcontact.ToNexusId();

            // when
            var response = MdmService.Get<SettlementContact>(nexusId);
        ...
        public void ShouldGetMapping()
        {
            ...
            // when
            var response = MdmService.GetMapping<SettlementContact>(id, nexusId => nexusId.SystemName == "Endur");

            // then
            Assert.IsTrue(response.IsValid, "###Error : " + response.Code + " : " + (response.Fault == null ? string.Empty : response.Fault.Message + " : " + response.Fault.Reason));
            Assert.AreEqual(settlementcontact.Identifiers.First(i => i.SystemName == "Endur").Identifier, response.Message.Identifier);
        }
        ...
        public void ShouldMap()
        {
            ...
            // when
            var response = MdmService.Map<SettlementContact>(id, "Endur");
        ...
        {
            // given
            var sourceIdentifier = settlementcontact.Identifiers.First(i => i.SystemName == "Endur");

            // when
            var response = MdmService.CrossMap<SettlementContact>(sourceIdentifier, "Trayport");

            // then
            Assert.IsTrue(response.IsValid, "###Error : " + response.Code + " : " + (response.Fault == null ? string.Empty : response.Fault.Message + " : " + response.Fault.Reason));
            Assert.AreEqual(settlementcontact.Identifiers.First(i => i.SystemName == "Trayport").Identifier, response.Message.Mappings[0].Identifier);
        }
    }
}
