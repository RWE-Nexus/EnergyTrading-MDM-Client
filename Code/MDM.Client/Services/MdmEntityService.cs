namespace EnergyTrading.MDM.Client.Services
{
        ...
        where TContract : class, IMdmEntity
    {
        ...
        public WebResponse<NexusId> CreateMapping(int id, NexusId identifier)
        {
            ...
            var response = this.Create<NexusId, MappingResponse>(uri, mapping);
            if (response.IsValid)
            {
        ...
        public WebResponse<TContract> Get(NexusId identifier, DateTime? validAt)
        {
            ...
            if (identifier.IsNexusId || identifier.SystemName == SourceSystemNames.Nexus)
            {
                int id;
        ...
        public WebResponse<TContract> Update(int id, TContract contract, string etag)
        {
            ...
            if (contract.Nexus != null && contract.Nexus.StartDate.HasValue)
            {
                queryString = string.Format(this.validAtParam, contract.Nexus.StartDate.Value.ToString(DateFormatString));
            }
        ...
        private WebResponse<TContract> AcquireEntity(NexusId sourceIdentifier, DateTime? validAt)
        {
